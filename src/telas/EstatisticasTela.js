...
import { View, Text, StyleSheet, ScrollView, SafeAreaView, Dimensions, Share, TouchableOpacity } from 'react-native';
...
const EstatisticasTela = () => {
  ...
  }, [boletos, boletosFiltrados, anoSelecionado]);

  const dadosGraficoPizzaPorEmissor = useMemo(() => {
    const boletosPagos = boletosFiltrados;
  ...
  const dadosGraficoPizzaPorPagador = useMemo(() => {
    const boletosPagos = boletosFiltrados.filter(b => b.pagador);
    if (boletosPagos.length === 0) return [];
    const totalPago = boletosPagos.reduce((acc, b) => acc + (b.valorPago || 0), 0);
    if (totalPago === 0) return [];
    const gastosPorPagador = boletosPagos.reduce((acc, boleto) => {
      acc[boleto.pagador] = (acc[boleto.pagador] || 0) + (boleto.valorPago || 0);
      return acc;
    }, {});
    const gerarCor = () => `rgba(${Math.floor(Math.random() * 255)}, ${Math.floor(Math.random() * 155) + 100}, ${Math.floor(Math.random() * 155) + 100}, 1)`;
    return Object.entries(gastosPorPagador)
      .map(([nome, valor]) => ({
        name: nome.length > 15 ? `${nome.substring(0, 12)}...` : nome,
        population: parseFloat(valor.toFixed(2)),
        color: gerarCor(),
  ...
  const dadosGraficoBarras = useMemo(() => {
    ...
    const labels = Object.keys(pagamentosPorMes).slice(-12);
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      await Share.share({
        message: dadosEmJson,
        title: `Backup Boletos - ${new Date().toLocaleDateString('pt-BR')}.json`
      });
    } catch (error) {
      Toast.show({ type: 'error', text1: 'Erro', text2: 'Não foi possível compartilhar o backup.' });
    }
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.contentContainer}>
        ...
        <View style={styles.containerFiltros}>
          <TouchableOpacity style={styles.botaoFiltro} onPress={() => setModalAnoVisivel(true)}>
            <Ionicons name="calendar-outline" size={20} color={tema.cores.primaria} />
            <Text style={styles.textoBotaoFiltro}>
              {anosOpcoes.find(opt => opt.value === anoSelecionado)?.label || 'Ano'}
            </Text>
            <Ionicons name="chevron-down" size={20} color={tema.cores.primaria} />
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.botaoFiltro, anoSelecionado === 'todos' && styles.botaoFiltroDesabilitado]}
            onPress={() => setModalMesVisivel(true)}
            disabled={anoSelecionado === 'todos'}
          >
            <Ionicons name="time-outline" size={20} color={anoSelecionado === 'todos' ? tema.cores.cinza : tema.cores.primaria} />
            <Text style={[styles.textoBotaoFiltro, anoSelecionado === 'todos' && { color: tema.cores.cinza }]}>
              {MESES_OPCOES.find(opt => opt.value === mesSelecionado)?.label || 'Mês'}
            </Text>
            <Ionicons name="chevron-down" size={20} color={anoSelecionado === 'todos' ? tema.cores.cinza : tema.cores.primaria} />
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.containerGrafico}>
          <Text style={styles.tituloGrafico}>Evolução das Despesas</Text>
          {(dadosGraficoBarras.labels?.length || 0) > 0 ? (
            ...
            <Text style={styles.textoVazio}>Nenhum boleto pago para exibir a evolução.</Text>
          )}
        </View>
        ...
        <View style={styles.containerGrafico}>
          ...
          {dadosGraficoPizzaPorEmissor.length > 0 ? (
            <>
              <PieChart
                data={dadosGraficoPizzaPorEmissor}
                width={screenWidth - tema.espacamento.medio * 2}
                height={220}
                chartConfig={chartConfig}
                accessor={"population"}
                backgroundColor={"transparent"}
                center={[(screenWidth - tema.espacamento.medio * 2) / 4, 0]}
                hasLegend={false}
              />
              <View style={styles.legendContainer}>
                {dadosGraficoPizzaPorEmissor.map(item => (
        ...
        <View style={styles.containerGrafico}>
          <Text style={styles.tituloGrafico}>Gastos por Pagador</Text>
          {dadosGraficoPizzaPorPagador.length > 0 ? (
            <>
              <PieChart
                data={dadosGraficoPizzaPorPagador}
                width={screenWidth - tema.espacamento.medio * 2}
                height={220}
                chartConfig={chartConfig}
                accessor={"population"}
              ...
              />
              <View style={styles.legendContainer}>
                {dadosGraficoPizzaPorPagador.map(item => (
                  <View key={item.name} style={styles.legendItem}>
                    <View style={[styles.colorSwatch, { backgroundColor: item.color }]} />
                    <Text style={styles.legendText}>{item.name} ({item.percentage}%)</Text>
                  </View>
          ...
          ) : ( <Text style={styles.textoVazio}>Nenhum boleto pago com pagador especificado.</Text> )}
        </View>
        ...
        <View style={styles.fullWidthCard}>
          ...
          <Text style={styles.fullWidthValue}>
            R$ {dadosEstatisticos.totalPago.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </Text>
        </View>
        ...
        <View style={styles.fullWidthCard}>
          ...
          <Text style={styles.fullWidthValue}>
            R$ {dadosEstatisticos.totalPendente.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </Text>
        </View>
      ...
      <ModalFiltro
        ...
        aoFechar={() => setModalAnoVisivel(false)}
        opcoes={anosOpcoes}
        filtroAtual={anoSelecionado}
        onSelecionarFiltro={(ano) => {
      ...
      <ModalFiltro
        ...
        aoFechar={() => setModalMesVisivel(false)}
        opcoes={MESES_OPCOES}
        filtroAtual={mesSelecionado}
        onSelecionarFiltro={setMesSelecionado}
      />
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  containerGrafico: {
    ...
    alignItems: 'center',
    elevation: 3,
    paddingHorizontal: tema.espacamento.pequeno,
  },
  tituloGrafico: {
    ...tema.tipografia.subtitulo,
    color: tema.cores.primaria,
  ...
  legendContainer: {
    width: '100%',
    paddingHorizontal: tema.espacamento.grande,
    marginTop: tema.espacamento.medio,
  },
  legendItem: {
    flexDirection: 'row',
